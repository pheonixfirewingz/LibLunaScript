...
class Parser
{
  ...
  private:
    ...
    inline bool isValidFuncCall(const std::vector<lexToken> &lexer, const uint32_t i) const noexcept
    {
        if (lexer[i].token == T_IDENTIFIER && lexer[i + 1].token == T_L_CURLY)
            return  true;
        return false;
    }
    ...
    const ASTBinaryExpression *parseBinaryExpr(const uint8_t precedence_inflator, const std::vector<lexToken> &lexer,
                                               uint32_t *i) noexcept;
    const ASTExpression *parseVar(const std::vector<lexToken> &lexer, uint32_t *i,bool is_global = false) noexcept;
